...
function describeHttpFailure(status: number, statusText: string, bodyText: string): string {
  ...
  if (!excerpt) return `CN LLM Proxy request failed with HTTP ${status} ${statusText}`.trim();
  const clipped = excerpt.length > 280 ? `${excerpt.slice(0, 279)}...` : excerpt;
  return `CN LLM Proxy request failed with HTTP ${status} ${statusText}: ${clipped}`.trim();
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const baseUrl =
    ...
    (process.env.CN_LLM_PROXY_BASE_URL ?? process.env.LLM_12AI_BASE_URL ?? "https://new.12ai.org/v1").replace(
      /\/+$/,
      "",
    );
  const apiKey =
  ...
  const model =
    ...
    (process.env.CN_LLM_PROXY_MODEL ?? process.env.LLM_12AI_MODEL ?? "gemini-2.5-flash").trim();
  const vendor = asString(config.vendor, "").trim() || "cn_llm_proxy";
  ...
  if (!baseUrl) {
    return {
      ...
      signal: null,
      timedOut: false,
      errorMessage: "CN LLM Proxy requires adapterConfig.baseUrl or CN_LLM_PROXY_BASE_URL / LLM_12AI_BASE_URL.",
      provider: "cn_llm_proxy",
      model: model || null,
  ...
  if (!apiKey) {
    return {
      ...
      signal: null,
      timedOut: false,
      errorMessage: "CN LLM Proxy requires adapterConfig.apiKey or CN_LLM_PROXY_API_KEY / LLM_12AI_API_KEY.",
      provider: "cn_llm_proxy",
      model: model || null,
      billingType: "metered_api",
    };
  ...
  if (!model) {
    return {
      ...
      signal: null,
      timedOut: false,
      errorMessage: "CN LLM Proxy requires adapterConfig.model or CN_LLM_PROXY_MODEL / LLM_12AI_MODEL.",
      provider: "cn_llm_proxy",
      model: null,
      billingType: "metered_api",
    };
  ...
  try {
    await ctx.onLog("stdout", `[paperclip] CN LLM Proxy request: POST ${endpoint} model=${model}\n`);
    ...
    const errorMessage = timedOut
      ? `CN LLM Proxy request timed out after ${timeoutSec}s.`
