...
export async function testEnvironment(
  ...
  if (!baseUrl) {
    checks.push({
      code: "bitexingai_base_url_missing",
      level: "error",
      message: "BitexingAI requires adapterConfig.baseUrl.",
      hint: "Set adapterConfig.baseUrl to the BitexingAI OpenAI-compatible API base URL.",
    });
  ...
  try {
    ...
    checks.push({
      code: "bitexingai_base_url_invalid",
      level: "error",
  ...
  if (url.protocol === "https:") {
    ...
    checks.push({
      code: "bitexingai_base_url_protocol_invalid",
      level: "error",
  ...
  if (!apiKey) {
    checks.push({
      code: "bitexingai_api_key_missing",
      level: "error",
      message: "BitexingAI requires adapterConfig.apiKey.",
      hint: "Set the BitexingAI bearer token in adapterConfig.apiKey.",
    });
  } else {
    checks.push({
      code: "bitexingai_api_key_present",
      level: "info",
  ...
  if (!model) {
    checks.push({
      code: "bitexingai_model_missing",
      level: "error",
      message: "BitexingAI requires adapterConfig.model.",
      hint: "Choose a model from the /models response or the static seed list.",
    });
  } else {
    checks.push({
      code: "bitexingai_model_present",
      level: "info",
  ...
  try {
    ...
    if (!response.ok) {
      ...
      checks.push({
        code: "bitexingai_models_probe_failed",
        level: "error",
        message: "BitexingAI /models probe failed.",
    ...
    checks.push({
      code: "bitexingai_models_probe_ok",
      level: "info",
      message: `BitexingAI /models probe succeeded${discovered.length > 0 ? ` with ${discovered.length} model(s)` : ""}.`,
    });
  } catch (error) {
    const timedOut =
    ...
    checks.push({
      code: timedOut ? "bitexingai_models_probe_timeout" : "bitexingai_models_probe_error",
      level: "error",
      message: timedOut
        ? `BitexingAI /models probe timed out after ${timeoutSec}s.`
        : "BitexingAI /models probe failed.",
